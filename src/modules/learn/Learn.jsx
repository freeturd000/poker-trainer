        ...
        <ol className="grid grid-cols-1 gap-3">
          {PHASES.map((p) => (
            <li key={p.id}>
              <button
                onClick={() => onOpen(p.id)}
                className="flex w-full items-center gap-4 pt-card p-4 text-left transition hover:bg-white sm:p-5"
              >
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-lg font-bold tabular-nums text-white">
                  {p.number}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-base font-bold text-emerald-900 sm:text-lg">{p.title}</span>
  ...
  return (
    <div className="pt-screen">
      ...
      <div className="w-full max-w-prose">
        <button
          onClick={onBack}
          className="mb-4 inline-flex items-center gap-1 text-sm font-semibold text-emerald-200 hover:text-white"
        >
        ...
        <article className="pt-card p-5 sm:p-8">
          <header className="mb-6 border-b border-gray-100 pb-5">
            <div className="pt-eyebrow text-emerald-600">Phase {phase.number}</div>
            <h1 className="mt-1 text-2xl font-bold text-emerald-900 sm:text-3xl">{phase.title}</h1>
            <p className="mt-2 text-base leading-relaxed text-gray-600">{phase.description}</p>
